...
export const config = {
  matcher: [
    ...
    // (video extensions included so public demo media loads signed-out)
    "/((?!_next|[^?]*\\.(?:html?|css|js(?!on)|jpe?g|webp|png|gif|svg|ttf|woff2?|ico|csv|txt|docx?|xlsx?|zip|webmanifest|mp4|webm|mov)).*)",
