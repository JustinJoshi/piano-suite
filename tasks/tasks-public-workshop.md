...
- [ ] A.0 Branch
  - [ ] A.0.1 `git checkout -b cursor/public-workshop-<suffix>` from current
    main (or the overhaul-plan branch if this lands in the same PR)

- [ ] A.1 Extract and test the public-route matrix
  - [ ] A.1.1 Write `lib/__tests__/public-routes.test.ts` FIRST:
  ...
  - [ ] A.1.2 Extract `isPublicPath` to `lib/public-routes.ts` from the
...
- [ ] A.5 Docs + gate
  - [ ] A.5.1 Update the route table in `docs/PROJECT_HISTORY.md` and the
        proxy comment. Mention in `README.md` only if the public-route
        sentence is now wrong.
  - [ ] A.5.2 `npm run lint && npm run test:unit:run && npm run build`
  - [ ] A.5.3 Manual: incognito, `/` → "Enter the Workshop" → starter picker,
        no account. Time it.
