...
| `lib/analytics.ts` | PostHog wrapper (`ANALYTICS_EVENTS`: `drill_started`, `drill_completed`, `pro_waitlist_click`, `door_clicked`, `page_created`, `block_added`); no-op without key; mirrors to `window.__analyticsEvents` for e2e. Emit via `captureEvent`, never per-page — pure store functions must never call it inline (React StrictMode double-invokes updaters); instead return a `PendingAnalyticsEvent` and have the caller fire it once outside the state updater via `capturePending` (see the `*WithEvent` wrappers in `lib/custom-practice-storage.ts`) |
