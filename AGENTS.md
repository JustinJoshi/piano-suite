...
9. **Wiring problems are guidance, not enforcement.** `PracticePageEditor` memoises `validateArrangement` and passes `issuesByBlockId: Map<blockId, WiringIssue[]>` once through `WorkshopGrid` to `WorkshopTile`, which renders a muted plain-language notice mapped over the three-member `WiringIssue` union (`unmet_requirement`, `orphan_transform`, `unscored_page`). A tile with issues still renders and the page still saves — the notice explains, it never blocks.
