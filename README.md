...
- A **piano-themed design system** with two materials. **The roll**, the default: every public page is a player-piano roll, cream paper hanging from a brass tracker bar whose slots light for every note that sounds (a key, a MIDI keyboard, a hole on the paper), ending on the red felt of the key slip. **The studio**: a dark brass-and-ebony Amber preset, an **Ivory** light theme and five more brand presets for the workspace. Piano keys are always ivory and ebony, primary buttons are the action colour, and Play / Explore / Learn keep their door hues everywhere. Pick a workspace theme at `/settings/theme`; the rules live in `DESIGN-PRINCIPLES.md`.
- A **landing page that is one long piano roll** (`components/welcome/roll/*`). The first thing on the paper is a real four-chord drill (I–V–vi–IV) you can play with the mouse, a touchscreen, the computer keyboard, or MIDI, and your notes are punched into a small roll as you play. Real music is punched between the sections and plays as it scrolls past the tracker bar once you turn the sound on. The sections read the app's real registries: the four drills, the two guided routes, the ten ready-made pages, the articles. Your own runs land in a small ledger on the page, and the roll ends on *Fine* and *Da capo*. The ambient math atmospheres (Chladni, Quasiperiodic, MIDI Ripple, Julia, Lissajous) now live in the workspace; they are switched off behind the roll's opaque paper.
- An **Articles section** (`/articles`) with a listing page and statically generated article pages (`/articles/[slug]`). Articles are authored in Markdown with YAML frontmatter, rendered with `react-markdown`, and styled with reusable Tailwind utility classes. Article pages sit on the roll with the shared tracker bar, so readers can navigate back to the home page and other sections at any time. A floating chat bubble on article pages links directly to the Practice Assistant so readers can ask questions about the content. A setup guide at `/articles/anki-ankiconnect-setup` walks new users through installing Anki and AnkiConnect.
- The **Workshop-first tools workspace**: `/tools` lands on the **Workshop** (`/tools/workshop`), the core of the app, where you compose your own practice pages and can jump into ready-made drills — open to signed-out visitors, no account needed. The sidebar groups navigation into **Workshop → Ready-made drills → Progress → Labs** (labs collapse behind one toggle; fixed on desktop; off-canvas drawer with menu control on mobile).
- A **first-time onboarding strip** on `/tools`: the page renders immediately with a dismissible in-flow strip, and the six-slide tour — active recall & spaced repetition, hand/body care, managing frustration through focused/diffuse practice — opens solely via the strip's **Take the tour** button. `?onboarding=reset` / `?onboarding=instant` and the settings-page Replay control still clear completion / force instant mode; reset restores the strip rather than auto-opening the overlay. State is stored in `localStorage`; the check is `e2e/first-visit.spec.ts`.
...
- **Guided routes** (`/routes`, public) — the zero-to-playing starter package: pick **Music theory** (set up Anki with the bundled chord decks, drill chords with Anki Sync, then progressions) or **Finger flexibility** (hand care, Technique tracker sessions, Arpeggios). Each route is a short checklist with one clear action per step, progress saved on-device, and a final step that builds your Workshop practice page for you. A quiet button on the Anki step copies a self-contained prompt (deck files included) that a computer assistant can follow to do the whole Anki setup. The Workshop's "How do you want to start?" picker surfaces both routes up top as cards with live progress, ahead of the ready-made practice pages.
 - A **Workshop** (`/tools/workshop`) where your whole practice page is a full-page grid you arrange yourself — drag tiles, resize them from their corner handle, and make room for what matters (the grid appears only while you drag). Feature blocks — metronome, drill timer, rest timer, chord sets, **scale runs**, **key cycles**, **chord progressions**, **session stats**, ready-made drill shortcuts, an on-screen keyboard — are all movable tiles. A drill-timer block's countdown/break/multi-rep settings drive the page's round shape, and the page's **drill block** (chord set, scale run, key cycle, or progression) drives its targets and its exact-match / grade thresholds. Only one drill block is live per page — the first one — and any others say so rather than fighting over the runtime, so a full warm-up → technique → theory session is a few linked pages rather than one crowded one. Source blocks feed display blocks through the page's practice stream: a Chord library or Scale library drives whatever Target display or Note roll shares the page, a Rhythm pattern times it, and a Transport block makes the page clock-advanced (one bar per target; a missed bar counts as a miss). A page with a target display, drill timer, or transport but no drill block now grades itself against the stream too — it advances through the stream's grouped targets and logs pitch-class reps as you play them (scoring stays pitch-class only; octave and voicing are not checked). A **Block library** tab (`/tools/workshop/blocks`) groups blocks by kind — interactive ones preview live, sources and transforms sit in quiet single-line rows showing a sample of the notes they emit — and every block's **About panel** states in plain language what it needs from the page you have (a transport, a source of notes, a way to play); add one with its plus button. Once blocks are on your page, a tile that doesn't fit yet — a transform with nothing to transform, or a block still waiting on a source of notes — says so in plain language right on the tile, and your page still plays and saves while you fix it. Switch between custom pages from the pages menu; per-block settings live behind each tile's gear. The Workshop is keyboard-first: **Ctrl/Cmd+K** opens a command palette (add blocks, switch pages, focus or open settings for a tile), **?** lists every shortcut, **/** opens the block library, arrows reorder tiles once grabbed from a tile's drag handle, and tile size is editable from the gear panel's Width/Height sliders. Pages persist in `localStorage` (Free) and practice events appear in the Tracking dashboard's Workshop tab (Pro syncs to Convex). And for the curious — or for a computer assistant helping you build — the Workshop publishes a machine-readable catalogue of every building block at `/api/blocks`.
- **Ready-made practice pages** built from those blocks, offered by the Workshop's starter picker and the Marketplace: a ten-minute warm-up, scale of the day, five-finger foundations, circle-of-fourths chords, ii-V-I in every key, 12-bar blues, the I-V-vi-IV pop loop, a minor-11th lap, and a modes tour — plus a Hanon-style cell warmup on a falling-note roll, a rootless ii-V-I that cycles all twelve keys, and a pentatonic improvisation scope. Each maps to a drill self-taught pianists are actually told to practice, and each is a starting point you can rearrange.
- A public **Marketplace** (`/marketplace`) — community practice pages you can try and **fork into your own workshop** with one button (signed-out forks land in this browser's localStorage). It ships with **featured pages** so it is never an empty room — now including a Hanon cell that watches itself fall toward the hit line, a rootless ii-V-I with no clock attached, and a pentatonic scope that reads your playing instead of grading it. **Publishing is free for any signed-in user** — the Workshop share menu puts your page in the community list with attribution (up to 25 published pages each; cross-device *sync* of your library stays Pro). Readers can report a page, and pages auto-hide at three reports. `/workshop` still works as a legacy redirect.
- **Learn a real piece** — the featured **"Learn a piece, one section at a time"** page turns any MIDI file into a practice routine. Upload a piece (a `.mid` file you already have), and it loops the first four bars while a gentle tempo ramp raises the speed a little each run. Section loops stay anchored to the file's own bar lines, so slowing or speeding the tempo changes only the pace — the same notes are always selected. Falling notes show what is coming, and the on-screen keyboard rings what you play — no MIDI keyboard needed to start. Fork the page and change the loop, the tempo, or the hand filter (pick which MIDI tracks play left/right) to make it yours. Publishing from the Workshop share menu puts your page in the community list with attribution; `/workshop` still works as a legacy redirect.
- **Ready-made drills are public** — Chord Drill, Arpeggios, Root Cycling, and Progression all work without signing in, with a built-in **on-screen keyboard** block (click, touch, or type A W S E D…) so "Play now" works for visitors with no MIDI controller. Signed-out practice writes to on-device history. The **Workshop and its block library are public too** (Phase 2.1): build and keep practice pages with no account — pages save in this browser, and a quiet header hint reminds you that signing in adds cross-device sync. The landing CTA leads through `/start`, whose **Play** door opens a public drill immediately and whose **Build** door opens the Workshop.
...
- A public **Pricing** page (`/pricing`) — pre-launch it shows the **Founding Pro waitlist** (billing off via `BILLING_ENABLED` in `lib/billing.ts`); at launch it flips to Clerk `<PricingTable />` for Free vs Pro (cloud sync + practice float panel — publishing to the Marketplace is free and not part of Pro). Manage subscriptions at `/settings/billing`. Legal pages: `/terms`, `/privacy`.
