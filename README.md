...
- **A friendly front door**: land on `/`, pick one of three doors — **Play** (guided routes and ready-made drills), **Build** (the Workshop), or **Learn** (articles). The pillars of healthy piano learning — active recall & spaced repetition, hand care, and managing frustration — live at `/learn/practice-pillars` and as an article, not behind a gate.
...
- The **Workshop-first tools workspace**: the **Workshop** (`/tools/workshop`) is the core of the app, and it is **public** — build and run practice pages without an account (pages persist in your browser; sign-in buys sync and publishing, never access). The sidebar collapses to four sections: **Workshop** (with guided routes and ready-made drills nested as starting points), **Shelf**, **Progress**, and **Settings** — fixed on desktop, off-canvas drawer on mobile.
- A first run in the Workshop greets you with starter templates and three dismissible hints; nothing gates the editor. The pillars deck is replayable from `/settings` and `/learn/practice-pillars`.
...
- **Guided routes** (`/routes`, public) — the zero-to-playing starter package: pick **Music theory** (set up Anki with the bundled chord decks, drill chords with Anki Sync, then progressions) or **Finger flexibility** (hand care, Technique tracker sessions, Arpeggios). Each route is a short checklist with one clear action per step, progress saved on-device, and a final step that builds your Workshop practice page for you. A quiet button on the Anki step copies a self-contained prompt (deck files included) that a computer assistant can follow to do the whole Anki setup. The Workshop's "How do you want to start?" picker surfaces both routes up top as cards with live progress, ahead of the ready-made practice pages.
 - A **Workshop** (`/tools/workshop`) where your whole practice page is a full-page grid you arrange yourself — drag tiles, resize them from their corner handle, and make room for what matters (the grid appears only while you drag). Feature blocks — metronome, drill timer, chord sets, ready-made drill shortcuts — are all movable tiles. The **Shelf** (`/tools/workshop/marketplace`) is the official block library — every feature previews live; add one with its plus button. Labs remain reachable from the shelf until they become blocks. Switch between custom pages from the pages menu; per-block settings live behind each tile's gear. Pages persist in `localStorage` (Free) and practice events appear in the Tracking dashboard's Workshop tab (Pro syncs to Convex).
- A **Community gallery** (`/workshop`, public) — practice pages other pianists have published. Open one, run it without an account, and **Save a copy to my Workshop** in one click: unsigned visitors copy it into their browser; signed-in users also record the fork server-side (attribution: "Based on a community drill"). Publishing your own page to the gallery is a Pro feature (Share → Publish).
