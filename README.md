...
2. Fill in real credentials:
   ...
   - Optional: auth bypass is **on by default**. Set `NEXT_PUBLIC_AUTH_DISABLED=false` to require Clerk sign-in again (Convex saves still need a session either way). Restart `npm run dev` after changing it.
